...
export function* confirmStripePayment({card, currentUser ,stripe , client_secret}: StripeFormFieldCSecret ){
...
export function* paymentIntentInit({payload:{amount, card, currentUser, stripe}}: CreateOrderStart){
  ...
  if(paymentIntent !== undefined && paymentIntent.client_secret !== null){
    ...
    yield* call(confirmStripePayment, {card, currentUser, stripe, client_secret});
  }
  else{
    alert(error?.message);
    yield* put(orderFailed(error as any));
